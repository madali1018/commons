package com.mada.commons.util.zookeeper;

import com.alibaba.fastjson.JSONObject;
import com.mada.commons.configuration.ConfigurationUtil;
...
import com.mada.commons.util.zookeeper.callback.IZkConnectionListenerCallback;
import com.mada.commons.util.zookeeper.callback.IZkInfrastructureListenerCallback;
import com.mada.commons.util.zookeeper.callback.IZkServiceConfigListenerCallback;
...
public class ZkUtil {
    ...
    public static void connect(ServiceEnum curServ, ServiceEnum[] depServs) {
        ...
        if (Objects.isNull(currentService)) {
            ...
            LOGGER.info("Connect to com.mada.zookeeper (host: {}).", ZOOKEEPER_HOST);
    ...
    public static void disconnect() {
        if (Objects.nonNull(currentService)) {
            currentService = null;
            client.close();
            LOGGER.info("Disconnect com.mada.zookeeper (host: {}).", ZOOKEEPER_HOST);
        }
    }
...
//                case SUSPENDED:
    ...
    private static void deleteConnection(ServiceEnum serviceEnum) {
        ...
        try {
            if (checkNode(path)) {
                ...
                //获取节点，没有时，忽略
                LOGGER.warn("No such node.node path:{}", path);
            }
        } catch (Throwable t) {
            throw new RuntimeException(t);
        }
    ...
    public static boolean isLeaderConnection() {
        ...
        if (CollectionUtil.isEmpty(connectionNodeEntityList)) {
            return false;
        }
